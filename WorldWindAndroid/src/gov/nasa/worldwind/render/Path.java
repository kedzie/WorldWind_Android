 ...
 * <p/>
 * Altitudes within the path's positions are interpreted according to the path's altitude mode. If the altitude mode is {@link AVKey#ABSOLUTE}, the altitudes
 ...
 * <p/>
 * Between the specified positions the path is drawn along a curve specified by the path's path type, either {@link AVKey#GREAT_CIRCLE},
 * {@link AVKey#RHUMB_LINE} or {@link AVKey#LINEAR}. (See {@link #setPathType(String)}.)
 * <p/>
 * Paths have separate attributes for normal display and highlighted display. If no attributes are specified, default attributes are used. See
 * {@link #DEFAULT_INTERIOR_COLOR}, {@link #DEFAULT_OUTLINE_COLOR}, and {@link #DEFAULT_HIGHLIGHT_COLOR}.
 * <p/>
 * When the path type is <code>LINEAR</code> the path conforms to terrain only if the follow-terrain property is true. Otherwise the path control points will be
 * connected by straight line segments.
 * <p/>
 * The terrain conformance of <code>GREAT_CIRCLE</code> or <code>RHUMB_LINE</code> paths is determined by the path's follow-terrain and terrain-conformance
 * properties. When the follow-terrain property is true, terrain conformance adapts as the view moves relative to the path; the terrain-conformance property
 ...
 * <p/>
 * The path positions may be shown by calling {@link #setShowPositions(boolean)} with an argument of <code>true</code>. This causes dots to be drawn at each
 * originally specified path position. Dots are not drawn at tessellated path positions. The size of the dots may be specified via
 * {@link #setShowPositionsScale(double)}. The dots are drawn only when the Path is within a threshold distance from the eye point. The threshold may be
 * specified by calling {@link #setShowPositionsThreshold(double)}. The dots are drawn in the path's outline material colors by default.
 * <p/>
 * The path's line and the path's position dots may be drawn in unique RGBA colors by configuring the path with a {@link PositionColors} (see
...
public class Path extends AbstractShape {
	...
	public static interface PositionColors {
		 ...
		 * guaranteed to be one of the same Position references passed to a path at construction or in a call to {@link Path#setPositions(Iterable)}.
		 * <p/>
		 * The specified <code>ordinal</code> denotes the position's ordinal number as it appears in the position list passed to the path. Ordinal numbers start
	...
	protected static class PathData extends AbstractShapeData {
		...
		/**
		 * The colors corresponding to each tessellated position, or <code>null</code> if the path's <code>positionColors</code> is <code>null</code>.
		 */
		...
		*/
		...
		/**
		 * Indicates the offset in number of floats to the first RGBA color tuple in <code>renderedPath</code>. This is <code>0</code> if
	 ...
	 * returned colors are applied to this path's line and the optional dots drawn at each position when <code>showPositions</code> is true, and override the
	 ...
	 * <p/>
	 * If this path is configured to tessellate itself by creating additional positions between the originally specified positions, an interpolated color is
	...
	protected boolean mustRegenerateGeometry(DrawContext dc) {
		if (this.getCurrentPathData() == null || this.getCurrentPathData().renderedPath == null) return true;
	...
	protected boolean doMakeOrderedRenderable(DrawContext dc) {
		...
		if (pathData.getReferencePoint() == null) return false;
		...
		if (pathData.tessellatedPositions == null || pathData.tessellatedPositions.size() < 2) return false;
		...
		if (pathData.renderedPath == null || pathData.renderedPath.limit() < 6) return false;
...
//		pathData.setExtent(this.computeExtent(pathData));
	 ...
	 * {@inheritDoc}
	 * <p/>
	 * Overridden to place this Path behind other ordered renderables when this Path is entirely located on the underlying terrain. In this case this Path must
	 * be drawn first to ensure that other ordered renderables are correctly drawn on top of it and are not affected by this Path's depth offset. If two paths
	 ...
	 * {@inheritDoc}
	 * <p/>
	 * If this Path is entirely located on the terrain, this applies an offset to the Path's depth values to to ensure it shows over the terrain. This does not
	 * apply a depth offset in any other case to avoid incorrectly drawing the path over objects it should be behind, including the terrain. In addition to
	 ...
	 * test. Since this Path is located on the terrain, the terrain already provides the necessary depth values and we can be certain that other ordered
...
//            dc.getCurrentProgram().vertexAttribPointer("vertexColor", 3, GLES20.GL_UNSIGNED_BYTE, false, 0, pickPositionColors);
	...
	protected void doDrawInterior(DrawContext dc) {
		int[] vboIds = this.getVboIds(dc);
		if (vboIds != null) this.doDrawInteriorVBO(dc, vboIds, this.getCurrentPathData());
		else {
			String msg = Logging.getMessage("GL.VertexBufferObjectNotInCache", this);
			Logging.warning(msg);
		}
	...
	protected void computePath(DrawContext dc, List<Position> positions, PathData pathData) {
		...
		if (AVKey.CLAMP_TO_GROUND.equals(this.getAltitudeMode())) path = this.computePointsRelativeToTerrain(dc, positions, 0d, path, pathData);
		else if (AVKey.RELATIVE_TO_GROUND.equals(this.getAltitudeMode())) path = this.computePointsRelativeToTerrain(dc, positions, null, path, pathData);
		else path = this.computeAbsolutePoints(dc, positions, path, pathData);
	 ...
	 */
	protected FloatBuffer computePointsRelativeToTerrain(DrawContext dc, List<Position> positions, Double altitude, FloatBuffer path, PathData pathData) {
		...
		if (path == null || path.capacity() < elemsPerPoint * numPoints) path = BufferUtil.newFloatBuffer(elemsPerPoint * numPoints);
		...
		for (Position pos : positions) {
			...
			if (extrudeIt) this.appendTerrainPoint(dc, pos, color, path, pathData);
		}
	 ...
	 * @return the buffer in which to place the computed points.
	 */
	protected FloatBuffer computeAbsolutePoints(DrawContext dc, List<Position> positions, FloatBuffer path, PathData pathData) {
		...
		if (dc.getVerticalExaggeration() != 1) {
			...
			for (Position pos : positions) {
				...
				if (this.isExtrude()) this.appendTerrainPoint(dc, pos, color, path, pathData);
			}
		} else {
			for (Position pos : positions) {
	 ...
	 * @param path
	 *            the path to append to. Assumes that the path has adequate capacity.
	 * @param pathData
	 *            the current globe-specific path data.
	 */
	protected void appendTerrainPoint(DrawContext dc, Position position, float[] color, FloatBuffer path, PathData pathData) {
		Vec4 referencePoint = pathData.getReferencePoint();
	...
	*/
	...
	protected void makePositions(DrawContext dc, PathData pathData) {
		...
		while (iter.hasNext()) {
			...
			if (this.isSmall(dc, ptA, ptB, 8) || !this.isSegmentVisible(dc, posA, posB, ptA, ptB)) this.addTessellatedPosition(posB, colorB, ordinalB, pathData);
			else this.makeSegment(dc, posA, posB, ptA, ptB, colorA, colorB, ordinalA, ordinalB, pathData);
	...
	*/
	 ...
	 *            the current globe-specific path data.
	 */
	protected void makeSegment(DrawContext dc, Position posA, Position posB, Vec4 ptA, Vec4 ptB, Color colorA, Color colorB, int ordinalA, int ordinalB, PathData pathData) {
		...
		double arcLength = this.getPathType() == AVKey.LINEAR ? ptA.distanceTo3(ptB) : this.computeSegmentLength(dc, posA, posB);
		if (arcLength <= 0 || (this.getPathType() == AVKey.LINEAR && !this.isFollowTerrain())) {
			if (!ptA.equals(ptB)) this.addTessellatedPosition(posB, colorB, ordinalB, pathData);
			return;
		}
	...
	protected void computeReferenceCenter(DrawContext dc) {
		...
		dc.getGlobe().computePointFromPosition(pos.latitude, pos.longitude, dc.getVerticalExaggeration() * pos.elevation, this.getCurrentPathData().referencePoint);
	}
	...
	protected double computeEyeDistance(DrawContext dc, PathData pathData) {
		...
		while (pathData.renderedPath.hasRemaining()) {
			...
			if (pathData.vertexStride > 3) pathData.renderedPath.position(pathData.renderedPath.position() + pathData.vertexStride - 3);
		}
	...
	protected Extent computeExtent(PathData current) {
		if (current.renderedPath == null) return null;
