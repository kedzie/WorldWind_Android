...
/**
 * Builds a list of {@link SurfaceTile} instances who's content is defined by a specified set
 * of {@link SurfaceObject} references. It's typically not necessary to use
 * SurfaceObjectTileBuilder directly. World Wind's default {@link gov.nasa.worldwind.SceneController} automatically
 * batches instances of {@link SurfaceObject} in a single SurfaceObjectTileBuilder.
 * Applications that need to draw basic surface shapes should use or extend {@link
 * SurfaceShape} instead of using SurfaceObjectTileBuilder directly.
 * <p/>
 * Surface tiles are built by calling {@link #buildTiles(gov.nasa.worldwind.render.DrawContext, Iterable)} (DrawContext, Iterable)} with an
 ...
 * SurfaceObject#render(gov.nasa.worldwind.render.DrawContext)}. This process may temporarily use the framebuffer to
 * perform offscreen rendering, and therefore should be called during the preRender method of a World Wind {@link
 * gov.nasa.worldwind.layers.Layer}. See {@link PreRenderable} for details. Once built, the
 * surface tiles can be rendered by a {@link SurfaceTileRenderer}.
 * <p/>
 ...
 * #setTileDimension(Point)} or {@link #setTileTextureFormat(int)}.
 * <p/>
 ...
 * Renderable} can use SurfaceObjectTileBuilder to draw a set of SurfaceObjects as follows:
 * <p/>
 * <code>
 * <pre>
...
public class SurfaceObjectTileBuilder
{
    ...
    protected static final int DEFAULT_TEXTURE_INTERNAL_FORMAT = GL_RGBA;
    /** The default OpenGL pixel format used to create surface tile textures. */
    protected static final int DEFAULT_TEXTURE_PIXEL_FORMAT = GL_RGBA;
    /**
     ...
     */
    protected static Map<Point, LevelSet> levelSetMap = new HashMap<Point, LevelSet>();
    /**
     ...
     */
    protected Map<Point, String> tileCacheNameMap = new HashMap<Point, String>();

    /**
     * Indicates the desired tile texture width and height, in pixels. Initially set to
     * <code>DEFAULT_TEXTURE_DIMENSION</code>.
     */
    protected Point tileDimension = new Point(DEFAULT_TEXTURE_DIMENSION, DEFAULT_TEXTURE_DIMENSION);
    /**
     ...
     */
    protected Point currentTileDimension;
     ...
     */
    public SurfaceObjectTileBuilder(Point tileTextureDimension, int tileTextureFormat, boolean useLinearFilter,
        boolean useMipmaps)
    {
     ...
     */
    public void setTileDimension(Point dimension)
    {
        if (dimension == null)
        {
            String message = Logging.getMessage("nullValue.DimensionIsNull");
    ...
    public List<SurfaceTile> buildTiles(DrawContext dc, Iterable<?> iterable)
    {
		...
		if(WorldWindowImpl.DEBUG)
			Logging.verbose("Built surface object tiles #" + tiles.size());
        return tiles;
    }
     ...
     */
    protected String getTileCacheName(Point tileDimension)
    {
     ...
     */
    protected Point computeTextureTileDimension(DrawContext dc)
    {
        // Force a square dimension by using the maximum of the tile builder's tileWidth and tileHeight.
        int maxSize = Math.max(this.tileDimension.x, this.tileDimension.y);
        ...
        if (maxSize > viewport.width)
            maxSize = (int)viewport.width;
        if (maxSize > viewport.height)
            maxSize = (int)viewport.height;
        ...
        return new Point(potSize, potSize);
    }
    ...
    /**
     * Updates each {@link gov.nasa.worldwind.render.SurfaceObjectTileBuilder.SurfaceObjectTile} in the {@link #currentTiles} list. This is
     * typically called after {@link #assembleTiles(gov.nasa.worldwind.render.DrawContext)} to update the assembled tiles.
     * <p/>
    ...
    protected void updateTiles(DrawContext dc)
    {
		...
		Logging.info("Updating tiles #" + currentTiles.size());
        ...
        this.rttSupport.setEnableFramebufferObject(
            this.tileTextureFormat == 0 || // Default format is GL_RGB8.
                this.tileTextureFormat == GL_RGB ||
                this.tileTextureFormat == GL_RGBA);

        Matrix projection = this.rttSupport.beginRendering(dc, 0, 0, this.currentTileDimension.x, this.currentTileDimension.y);
        try
        {
            for (SurfaceObjectTile tile : this.currentTiles)
            {
                this.updateTile(dc, tile, projection);
            }
        }
        finally
        {
     ...
     */
    protected void updateTile(DrawContext dc, SurfaceObjectTile tile, Matrix projection)
    {
        // Get the tile's texture from the draw context's texture cache. If null we create a new texture and update the
        // texture cache below.
        GpuTexture texture = tile.getTexture(dc.getTextureCache());
        ...
        try
        {
            ...
            dc.setValue(AVKey.SURFACE_TILE_DRAW_CONTEXT, this.createSurfaceTileDrawContext(tile, projection));
            ...
            if (tile.hasObjects())
            {
                ...
                {
					dc.setCurrentLayer(so.getLayer());
                    so.render(dc);
     ...
     * Name</th><th>Value</th></tr> <tr><td><code>GL_TEXTURE_MIN_FILTER</code></td><td><code>GL_LINEAR_MIPMAP_LINEAR</code>
     ...
     * <code>useLinearFilter</code> is false.</td></tr> <tr><td><code>GL_TEXTURE_MAG_FILTER</code></td><td><code>GL_LINEAR</code>
     * if <code>useLinearFilter</code> is true, <code>GL_NEAREST</code> if <code>useLinearFilter</code> is
     * false.</td></tr> <tr><td><code>GL_TEXTURE_WRAP_S</code></td><td><code>GL_CLAMP_TO_EDGE</code></td></tr>
     * <tr><td><code>GL_TEXTURE_WRAP_T</code></td><td><code>GL_CLAMP_TO_EDGE</code></td></tr>
     *
     ...
     */
    protected Object createSurfaceTileDrawContext(SurfaceObjectTile tile, Matrix projection)
    {
        return new SurfaceTileDrawContext(tile.getSector(), tile.getWidth(), tile.getHeight(), projection);
    }
     ...
     */
    protected LevelSet getLevelSet(Point tileDimension)
    {
        ...
        if (levelSet == null)
        {
            levelSet = createLevelSet(tileDimension.x, tileDimension.y);
     ...
     * surface object list at most once. See {@link gov.nasa.worldwind.render.SurfaceObjectTileBuilder.SurfaceObjectTile#addSurfaceObject(gov.nasa.worldwind.render.SurfaceObject,
    ...
    {
		Logging.info("Assembling tiles object #" + currentSurfaceObjects.size());
        ...
        for (SurfaceObject so : this.currentSurfaceObjects)
        {
            ...
            for (Sector s : sectors)
            {
                ...
                // surface objects.
                int firstRow = Tile.computeRow(dLat, s.minLatitude, latOrigin);
                int firstCol = Tile.computeColumn(dLon, s.minLongitude, lonOrigin);
                int lastRow = Tile.computeRow(dLat, s.maxLatitude, latOrigin);
                int lastCol = Tile.computeColumn(dLon, s.maxLongitude, lonOrigin);
                ...
                for (int row = firstRow; row <= lastRow; row++)
                {
                    ...
                    for (int col = firstCol; col <= lastCol; col++)
                    {
                        ...
                        SurfaceObjectTile tile = (SurfaceObjectTile) GpuTextureTile.getMemoryCache().get(tileKey);
                        if (tile == null)
                        {
                            tile = this.createTile(new Sector(p1, p2, t1, t2), level, row, col, tileCacheName);
                            GpuTextureTile.getMemoryCache().put(tileKey, tile);
                        }
		...
		if(WorldWindowImpl.DEBUG)
			Logging.verbose(String.format("Created %d top level surface shape tiles", topLevelTiles.size()));
     ...
     * parent's surface object list. See {@link gov.nasa.worldwind.render.SurfaceObjectTileBuilder.SurfaceObjectTile#getObjectList()}.
     * <p/>
     ...
     * See {@link #meetsRenderCriteria(gov.nasa.worldwind.render.DrawContext, gov.nasa.worldwind.util.LevelSet, gov.nasa.worldwind.util.Tile)}.
     *
...
//		if(WorldWindowImpl.DEBUG) Logging.verbose(String.format("Splitting tile to next level: %d", nextLevel.getLevelNumber()));
     ...
     */
    protected boolean intersectsFrustum(DrawContext dc, GpuTextureTile tile)
    {
     ...
     */
    protected boolean intersectsVisibleSector(DrawContext dc, GpuTextureTile tile)
    {
     ...
     * @param level     The tile's Level in a {@link gov.nasa.worldwind.util.LevelSet}.
    ...
    /**
     * Represents a {@link gov.nasa.worldwind.render.GpuTextureTile} who's contents is constructed by a set of surface
     * objects. The tile maintains a collection of surface objects that intersect the tile, and provides methods for to
     * modify and retrieve that collection. Additionally, the method {@link #getStateKey(gov.nasa.worldwind.render.DrawContext)} provides a
     * mechanism to uniquely identify the tile's current state, including the state of each intersecting surface
     * object.
     */
    protected static class SurfaceObjectTile extends GpuTextureTile
    {
         ...
         */
        @Override
        protected SurfaceObjectTile createSubTile(Sector sector, Level level, int row, int col)
        {
