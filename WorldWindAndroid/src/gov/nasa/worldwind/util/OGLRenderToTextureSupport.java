 ...
 * #clear(gov.nasa.worldwind.render.DrawContext, gov.nasa.worldwind.render.Color)} immediately after any call to {@link
 ...
 * passed in as an argument to the containing method.<br/> GpuTexture texture = TextureIO.newTexture(new
...
public class OGLRenderToTextureSupport
{
     ...
     * color buffer target.
     *
     * @return the GpuTexture currently set as the color buffer target, or null if none exists.
     */
     ...
     * @param texture the GpuTexture to use as the destination for GL commands affecting the color buffer. A null value is
     *                permitted.
     *
     * @throws IllegalArgumentException if the DrawContext is null.
     */
    public void setColorTarget(DrawContext dc, GpuTexture texture)
    {
        if (dc == null)
        {
    ...
    public void clear(DrawContext dc, Color color)
    {
		...
		color.premultiply();
		color.toArray4f(tmpArrayF, 0);
        glClearColor(tmpArrayF[0], tmpArrayF[1], tmpArrayF[2], tmpArrayF[3]);
     ...
     * @throws IllegalArgumentException if the DrawContext is null.
     */
    public Matrix beginRendering(DrawContext dc, int x, int y, int width, int height)
    {
        if (dc == null)
        {
            String message = Logging.getMessage("nullValue.DrawContextIsNull");
        ...
        this.stackHandler.pushAttrib(
				GL_COLOR_BUFFER_BIT  // For clear color.
						| GL_DEPTH_BUFFER_BIT // For depth test and depth mask.
						| GL_SCISSOR_BIT      // For scissor test and scissor box.
						| GL_VIEWPORT_BIT
		);   // For viewport state.
    ...
    protected void flushColor(DrawContext dc)
    {
        ...
        if (this.useFramebufferObject(dc))
        {
            if (this.colorTarget != null)
			{
				...
				glGenerateMipmap(this.colorTarget.getTarget());
			}
        }
        ...
        else
        {
            if (this.colorTarget != null)
            {
                this.copyScreenPixelsToTexture(dc, this.drawRegion.left, this.drawRegion.top,
                    this.drawRegion.width(), this.drawRegion.height(), this.colorTarget);
            }
        }
    }

    protected void copyScreenPixelsToTexture(DrawContext dc, int x, int y, int width, int height, GpuTexture texture)
	{
		int w = width;
		int h = height;
    ...
    protected void bindFramebufferColorAttachment(DrawContext dc, GpuTexture texture)
    {
