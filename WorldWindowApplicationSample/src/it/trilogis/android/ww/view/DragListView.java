...
public class DragListView extends ListView {
	...
	private static final int GRABBER_LEFT = 0;
	private static final int GRABBER_RIGHT = 1;
	...
	public DragListView(Context context, AttributeSet attrs, int defStyle) {
		...
		if (attrs != null) {
			...
			mItemHeightNormal = a.getDimensionPixelSize(R.styleable.DragListView_normal_height, 0);
			mItemHeightHalf = mItemHeightNormal / 2;
			mItemHeightExpanded = a.getDimensionPixelSize(R.styleable.DragListView_expanded_height, mItemHeightNormal * 2);
			grabberId = a.getResourceId(R.styleable.DragListView_grabber, -1);
			dragndropBackgroundColor = a.getColor(R.styleable.DragListView_dragndrop_background, 0x00000000);
			mRemoveMode = a.getInt(R.styleable.DragListView_remove_mode, -1);
			grabberAlign = a.getInt(R.styleable.DragListView_grabber_alignment, 0);
		...
		if (mDragListener != null || mDropListener != null) {
			switch (ev.getAction()) {
				case MotionEvent.ACTION_DOWN:
					...
					boolean grabberSelected = false;
					if (grabberAlign == GRABBER_LEFT) {
						grabberSelected = x < 64;
					} else if (grabberAlign == GRABBER_RIGHT) {
						grabberSelected = x > getWidth() - 64;
					}
					if (grabberSelected) {
		...
		if ((mDragListener != null || mDropListener != null) && mDragView != null) {
			...
			switch (action) {
				...
				case MotionEvent.ACTION_CANCEL:
					...
					stopDragging();
					Rect trashRect = null;
					if (mRemoveMode == TRASH && mTrashcan != null) {
						trashRect = new Rect();
						mTrashcan.getHitRect(trashRect);
					}
					if (mRemoveMode == SLIDE && ev.getX() > r.right * 3 / 4) {
						...
						unExpandViews(true);
					} else if (trashRect != null && trashRect.contains((int) ev.getX(), (int) ev.getY())) {
						if (mRemoveListener != null) {
							mRemoveListener.remove(mSrcDragPos);
						}
						unExpandViews(true);
